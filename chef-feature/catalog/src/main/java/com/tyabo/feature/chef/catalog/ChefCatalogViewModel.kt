...
class ChefCatalogViewModel @Inject constructor(
    ...
    val fetchedCatalog: StateFlow<Unit> = chefRepository.catalog.map { catalogResult ->
        when(catalogResult){
            is UiResult.Success -> {
                _catalogState.value = com.tyabo.feature.chef.catalog.ChefCatalogViewState.Catalog(catalog = catalogResult.data)
            }
            else -> {
                _catalogState.value = com.tyabo.feature.chef.catalog.ChefCatalogViewState.Loading
            }
        }
    }
        .stateIn(
    ...
    private val _catalogState = MutableStateFlow<com.tyabo.feature.chef.catalog.ChefCatalogViewState>(
        com.tyabo.feature.chef.catalog.ChefCatalogViewState.Loading
    )
